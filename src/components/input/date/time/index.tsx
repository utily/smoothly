...
export class SmoothlyInputDateTime implements ComponentWillLoad, Clearable, Input, Editable {
	...
	@Prop({ mutable: true }) value?: DateTime
	@Prop({ mutable: true }) open: boolean
	@Prop({ mutable: true }) max: DateTime
	@Prop({ mutable: true }) min: DateTime
	...
	@Method()
	async getValue(): Promise<DateTime | undefined> {
		return this.value
	}
	@Method()
	...
	dateTimeChange() {
		...
		if (
			...
			(DateTime.is(value) && DateTime.is(this.value) && DateTime.span(value, this.value, "minutes").minutes != 0)
		)
			this.value = value
	}
	@Watch("value")
	valueChange(next?: DateTime) {
		if (DateTime.is(next)) {
