...
export class SmoothlyAccordionItem {
	...
	@Event() smoothlySelect!: EventEmitter<{ name: string, checked: boolean }>
	@Event() smoothlyDeselect!: EventEmitter<{ name: string, checked: boolean }>
	@Watch("checked")
	checkedChanged(isChecked: boolean) {
		console.log(this.name + " " + isChecked)
		if (isChecked) {
	...
	componentDidLoad() {
		...
		if (summary.length > 0) {
			const onClick = (e: UIEvent) => {
				if (this.checked)
					this.smoothlyDeselect.emit({ checked: this.checked, name: this.name })
				else
					this.checked = true
				e.preventDefault()
			}
				...
				<summary>
					{
						...
						{this.name}{
							typeof(this.brand) == "string" ? <span class={this.brand.toLowerCase()}>{this.brand}</span> :
							Array.isArray(this.brand) ? this.brand.map(b => <span class={b.toLowerCase()}>{b}</span>) :
							[] }
				</summary>
				<fieldset disabled={!this.checked} name={this.name}>
					<slot />
				</fieldset>
			</details>
		)
